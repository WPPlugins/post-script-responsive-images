...
class postScript_responsive_images {
	...
	public function imageResize( $data = array( ) ){
		...
		$postScript_responsive_images = new postScript_responsive_images;
		$sizes = array_reverse($postScript_responsive_images->get_image_sizes());
	...
	public function createThumbnail( $html, $post_id, $post_thumbnail_id, $size, $attr ) {
		...
		$postScript_responsive_images = new postScript_responsive_images;
		$html .= $postScript_responsive_images->imageResize( $resizeAttr );
		$html .= ' /> ';
		...
		foreach( $img as $key => $image):
			...
			if($id):
				...
				if( !strpos(strtolower($src), '.gif') ):
					$postScript_responsive_images = new postScript_responsive_images;
					$imgNew[$count] .= $postScript_responsive_images->imageResize( $resizeAttr );
				endif;
			endif;
			$imgNew[$count] .= ' /> ';
		endforeach;
...
add_action( 'post_thumbnail_html', array('postScript_responsive_images','createThumbnail'),0,5 );
add_action('the_content', array('postScript_responsive_images','createContentImage'));
